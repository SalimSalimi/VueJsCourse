<template>
  <div class="card">
    <div class="card-header texte-center">
      <h3>Hello World</h3>
    </div>
    <div class="card-body">
      <div class="col-xs-12 col-sm-6 text-center">
        <button style="btn btn-primary btn-lg" @click="onAnswer(btnData[0].correct)">{{ btnData[0].answer }}</button>
      </div>
      <div class="col-xs-12 col-sm-6 text-center">
        <button style="btn btn-primary btn-lg" @click="onAnswer(btnData[1].correct)">{{ btnData[1].answer }}</button>
      </div>
      <div class="col-xs-12 col-sm-6 text-center">
        <button style="btn btn-primary btn-lg" @click="onAnswer(btnData[2].correct)">{{ btnData[2].answer }}</button>
      </div>
      <div class="col-xs-12 col-sm-6 text-center">
        <button style="btn btn-primary btn-lg"></button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    generateQuestions() {
      const firstNumber = this.generateRandomNumbers(1, 100);
      const secondNumber = this.generateRandomNumbers(1, 100)
      const modeNumber = this.generateRandomNumbers(1,2);
    ...
    generateRandomNumbers(min, max, except) {
      const rndNumber = Math.round(Math.random() * (max-min)) + min;
      if (rndNumber == except) {
        return this.generateRandomNumbers(min, max, except);
      }
      return rndNumber;
    },
    onAnswer(isCorrect) {
